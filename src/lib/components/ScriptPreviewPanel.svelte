...
<aside class="h-fit rounded-2xl border border-slate-800 bg-slate-900/80 p-4 backdrop-blur">
  <div class="mb-3 flex items-center justify-between">
    <h3 class="text-sm font-medium text-slate-100">脚本预览</h3>
    <span class="text-xs text-slate-400">{isGenerating ? "生成中..." : "已同步"}</span>
  </div>
  ...
  <pre class="max-h-[65vh] overflow-auto rounded-xl border border-slate-700 bg-slate-950/70 p-3 text-xs leading-5 text-slate-300"><code>{script}</code></pre>
</aside>
