...
UI Components (modules/*)
    ↓ user input
configStore.patch({...})  → stores/config.svelte.ts
    ↓ subscribe
App.svelte
    ↓ watch configStore
...
| `src/lib/stores/config.svelte.ts`        | 集中式状态管理，含 patch/reset/persist |
...
3. 在 `config.svelte.ts` 的 `hasModuleSelection` 添加判断逻辑
...
- 使用 `$stores/config.svelte` 和 `$types/config` 路径别名
